			...
			if(isset($_POST['type']) && $_POST['type']=="property")
			{
				if ($this->general_model->category_property_manipulate($_POST))
				{
					if($_POST['currentid'] ==0)
						$this->session->set_flashdata('message', 'Updated Ads Form');
					else
				...
				{
					if($_POST['currentid'] ==0)
						$this->session->set_flashdata('message', 'Updated Ads Form error. Please try later');
					else
...
/*-------------------------------------------------------------------------*/
	...
	function editextensionImage($id='')
	{
		if($id!='')
		{
			...
			{
				$this->session->set_flashdata('message_error', 'Product not exist. Please try later');
...
//			}
		if ($this->session->userdata['group']==1) {
			if(isset($_POST['type']) && $_POST['type']=="images")
			{
				if(isset($_POST['extension_image'])){
					...
					$this->session->set_flashdata('message', 'Updated Product Image');	    	
				}
				else
				{
					$this->session->set_flashdata('message_error', 'Updated Product Image error. Please try later');
				}
			}
			...
			else if (isset($_POST['type']) && $_POST['type']=="download")
			{
				if(isset($_POST['extension_download'])){
					...
					$this->session->set_flashdata('message', 'Updated Download of Product');	   
	        	}
	        	else
	        	{
	        		$this->session->set_flashdata('message', 'Updated Download of Product error. Please try later');
	        	}
			}
			...
			else
			{
				...
				{
					$this->session->set_flashdata('message', 'Updated Ads');	    	
				}
				else
				{
					$this->session->set_flashdata('message', 'Updated Ads error. Please try later');
				}
			}
		}
		redirect("/cpanel/extensions/");
	...
	function deleteextension($id=0)
	{
		...
		$this->db->trans_complete();
		$this->session->set_flashdata('message', 'Deleted Ads');
		}
		redirect("/cpanel/extensions/");
	}
...
/*---------------------------------------------------------------------------*/	
	...
	function newslettermanager()
	{
		...
		foreach($temp as &$te)
		{
			...
			{
				$te['template'] = "Update Product";
			}
			else if($te['type']==3)
			{
				$te['template'] = "Purchase Product";
			}
			else if($te['type']==4)
			{
				$te['template'] = "Comment";
